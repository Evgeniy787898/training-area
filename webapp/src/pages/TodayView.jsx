...
const TodayView = () => {
    ...
    const loadSession = useCallback(async (date) => {
        ...
        try {
            ...
            if (fallbackSession) {
                ...
                showToast?.({
                    title: STANDALONE_MODE ? 'Нет соединения с API' : 'Режим предпросмотра',
                    message: STANDALONE_MODE
                        ? 'Проверь URL сервера и заголовки авторизации. Сейчас показан офлайн-план.'
                        : 'Показываю офлайн-план. Подключи Telegram WebApp, чтобы получить точные данные.',
                    type: 'warning',
    ...
    const handleSaveSession = async () => {
        if (!state.session?.id) {
            showToast?.({
                title: demoMode ? (STANDALONE_MODE ? 'Нет соединения с API' : 'Демо режим') : 'Нет связанной тренировки',
                message: demoMode
                    ? (STANDALONE_MODE
                        ? 'Сейчас нет соединения с сервером. Исправь параметры доступа и повтори попытку.'
