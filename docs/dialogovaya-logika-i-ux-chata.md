...
- Команды «Сохрани…» и «справка по триггерам» обрабатываются движком `internalAssistantEngine`: заметки пишутся в `assistant_notes`, справка перечисляет быстрые команды.
- `services/history.js` унифицирует хранение истории между ботом и HTTP API, чтобы WebApp мог запрашивать / сохранять ответы без рассинхронизации.
- Фоновый монитор завершает сессию после 60 минут бездействия и отправляет предупреждающее сообщение.

## Форматы запросов
- Свободные запросы: «Перенеси тренировку на завтра», «Покажи прогресс за месяц», «Измени напоминание на 7:30».
- Командные запросы: `/start`, `/plan`, `/report`, `/stats`, `/settings`, `/setup`, `/help`.
- Мемориальные запросы: «Сохрани: заметка на завтра», «Справка по триггерам».
...
- Фолбэк: при неизвестном намерении `ConversationService` обращается к внутреннему движку и отдаёт структурированный ответ тренера.
...
- При простое > 60 минут: короткое уведомление «закрываю сессию» с ссылкой на последний запрос, после чего история очищается.
- Фолбэк-ответ при неизвестном интенте дружелюбно предлагает выбрать действие на клавиатуре и открывает WebApp кнопкой в inline-меню.
...
- **Квоты**: если очередь ответов перегружена (например, более 3 одновременных расчётов плана), бот сообщает «Сейчас много запросов, повторю через минуту» и ставит повтор в очередь.
...
- Если пользователь молчит >60 минут, бот отправляет уведомление о завершении и очищает историю («Закрываю эту сессию, пиши, когда будем продолжать.»).
