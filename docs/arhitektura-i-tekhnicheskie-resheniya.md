...
- **Политики RLS**: таблицы пользовательских данных (`profiles`, `training_sessions`, `exercise_progress`, `metrics`, `weekly_reviews`, `plan_versions`, `plan_version_items`, `operation_log`, `observability_events`) защищены политикой `using (auth.uid() = profile_id)` или через джойны на `profiles`. Для Edge Functions создаётся сервисная роль `service_role` с политикой `with check (true)` и ограничением на тип операции (например, только `insert` для логов). Анонимные ключи получают только права чтения агрегированных справочников (`capabilities`, `analytics_reports`).
