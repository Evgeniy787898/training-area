...
- **Встроенный движок**: ответы и планы формируются локально на основе `services/internalAssistantEngine.js`.
...
- **Очередь**: все вызовы движка и отправки сообщений помещаются в Supabase Edge Queue или аналогичный механизм с ретраями (экспоненциальная задержка, до 3 попыток).
...
- **Очереди**: для обработки пиковых нагрузок запросы движка кладутся в очередь Supabase Edge Functions. Максимальная длина очереди — 200 задач, метрики отслеживаются через Grafana; при превышении лимита включается деградационный режим (отправка статического плана без AI).
...
2. Ответы сохраняются, формируется начальный план внутренним движком, результат пишется в `training_sessions` на ближайшие 14 дней.
...
3. Протестировать сценарии движка и обновить шаблоны ответов при необходимости (описаны в README).
