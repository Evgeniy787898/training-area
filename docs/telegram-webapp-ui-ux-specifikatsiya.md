...
- **Контракты API**:
  - `GET /v1/sessions/{id}` → `200 OK` `{ "id": "uuid", "date": "2024-05-10", "status": "planned|done", "exercises": [{ "exercise_key": "pull_up", "target": { "sets": 4, "reps": 6 }, "actual": { "sets": 4, "reps": 7 }, "rpe": 7.5, "notes": "...", "state": "planned|in_progress|done" }], "trace_id": "uuid", "updated_at": "timestamp" }`. Ошибки: `404` (`session_not_found`), `409` (`version_conflict`).
  - `PUT /v1/sessions/{id}` (`session_marked_done`) → принимает `{ "status": "done", "completed_at": "timestamp", "exercises": [...] }`. Ответ `200` `{ "success": true, "decision": "advance|hold|regress", "next_steps": "string" }`.
  - `GET /v1/reports/{slug}` → `200` `{ "report": "volume_trend", "range": "30d", "chart_url": "https://...", "summary": "...", "generated_at": "timestamp" }`. Ошибки: `422` (`range_invalid`), `404` (`report_not_available`).
  - `PATCH /v1/profile/preferences` → тело `{ "notification_time": "07:30", "timezone": "Europe/Moscow", "pause_until": "2024-06-01" }`. Ответ `200` `{ "success": true, "effective_at": "timestamp", "pending_jobs": ["notify_daily"] }`. Ошибки: `400` (`invalid_timezone`), `409` (`conflict_with_existing_pause`).
  - Структуры ответов перечислены в `docs/api-schema.md` (обновлять синхронно).
- **Изменение настроек**:
...
- **Частично заполненные отчёты**: если пользователь отправляет только заметку или только RPE, интерфейс показывает подсказку «Добавь хотя бы один факт о выполнении (подходы, время или статус)». Разрешён режим «только заметка» при статусе `session.skipped`; в этом случае WebApp отправляет `status = 'skipped'`, `rpe = null`, `notes` обязательны. При пустом payload WebApp блокирует отправку.
- **Обработка ошибок**: при `validation_failed` подсвечиваются поля, выводятся конкретные сообщения (например, «RPE должно быть от 1 до 10»). При `conflict` предлагается «Обновить данные» (рефетч) или «Открыть в чате».
...
- **Управление токенами**:
  - `auth_token` (JWT) живёт 15 минут, содержит `user_id`, `iat`, `exp`, `scope`. WebApp запрашивает обновление через `expand.getAuthData` при `exp - now < 120 секунд`.
  - Ключ подписи HMAC ротируется каждые 90 дней. В payload передаётся `key_id`, чтобы сервер понимал, какой ключ использовать.
  - При невалидной подписи бот возвращает `401 auth_invalid_signature`, WebApp закрывает сессию, очищает sessionStorage и предлагает открыть WebApp заново.
  - При подозрении на компрометацию (3 ошибки подряд) бот создаёт `security_events` (`event_type = webapp_auth_failure`) с деталями устройства (`platform`, `tg_webapp_version`).
