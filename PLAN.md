...
Документация-источник: [Концепция и работа бота](docs/kontseptsiya-i-rabota-bota.md), [Архитектура и технические решения](docs/arhitektura-i-tekhnicheskie-resheniya.md), [Правила тренера и диалогов](docs/pravila-trenera-i-dialoga.md), [Диалоговая логика и UX чата](docs/dialogovaya-logika-i-ux-chata.md), [Внутренний движок тренера](docs/dvizhok-trenera.md), [Тренировочные программы](docs/trenirovochnye-programmy-i-progressii.md), [Telegram WebApp UI/UX спецификация](docs/telegram-webapp-ui-ux-specifikatsiya.md).
...
3. Утвердить технологический стек и интеграции:
   ...
   - внутренний движок тренера (`internalAssistantEngine`) как основной поставщик логики;
...
4. Закрепить ответственность:
   ...
   - инженер по диалогам — сценарии движка, безопасность коммуникаций;
...
1. Создание базовой инфраструктуры:
   ...
   - Планировщик задач: выбрать подход (Supabase Queue, BullMQ) для фоновых расчётов бота;
...
1. Структура проекта `server/bot`:
   ...
   - подключить Telegraf, Axios (для Supabase), инициализировать `internalAssistantEngine`.
...
3. Middleware и безопасность:
   ...
   - нормализация текста (удаление PII, проверка токсичности по [правилам тренера](docs/pravila-trenera-i-dialoga.md));
   - idempotency middleware для повторных апдейтов Telegram.
4. Интеграция с сервисом планирования:
   - создать модуль `services/planner.ts`, который использует `internalAssistantEngine` для генерации планов и анализа отчётов (см. [архитектуру](docs/arhitektura-i-tekhnicheskie-resheniya.md));
...
5. Подготовить модульные тесты для команд и заглушки движка (имитация ответов `internalAssistantEngine`).
...
2. NLU-роутер:
   - реализовать пайплайн из [архитектуры](docs/arhitektura-i-tekhnicheskie-resheniya.md): нормализация → fast-intent → fallback к правилам движка;
   - обучить лёгкую модель intents (например, `fastText`) на фразах из документации;
   - добавить unit-тесты и dataset regression.
3. Шаблоны и guardrails:
   - обновить набор шаблонов для планирования, мотивации, справочных ответов согласно [правилам тренера](docs/pravila-trenera-i-dialoga.md);
...
1. Шаблоны тренировок:
   - на базе [логики движка](docs/dvizhok-trenera.md) сформировать библиотеку микроциклов (подготовка, прогрессия, разгрузка);
...
4. Медиа и визуализация:
   ...
   - собрать набор статических иллюстраций для мотивационных сообщений (иконки, фоновые изображения);
...
3. Наблюдаемость:
   ...
   - настроить дешборды Grafana (модули: Telegram webhook latency, engine processing time, Supabase errors);
...
1. Тестирование:
   ...
   - провести интеграционные тесты: Supabase ↔ Bot ↔ внутренний движок (использовать staging);
...
3. Долгосрочная карта развития:
   ...
   - заложить процесс обновления правил движка (новые шаблоны, корректировка алгоритмов).
