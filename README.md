...
- 🤖 **AI-планирование** — генерация персональных планов и мгновенный анализ отчётов с помощью ChatGPT
...
WebApp включает пять основных экранов: «Сегодня», «Неделя», «Отчёт», «Прогресс» и новую страницу «Каталог», где собраны прогрессии упражнений с подсказками и историей. Все экраны синхронизированы с API (`/v1/sessions`, `/v1/reports`, `/v1/achievements`, `/v1/exercises`).
...
| Функция | Назначение |
| --- | --- |
| `update_plan` | Анализирует прогресс, вызывает AI для генерации плана на неделю и создаёт новую активную версию плана в базе. |
...
Не забудьте задать переменные окружения (`SUPABASE_SERVICE_ROLE_KEY`, `OPENAI_API_KEY`, `TELEGRAM_BOT_TOKEN`) в настройках проекта Supabase.
...
- Системные промпты для ChatGPT находятся в `server/services/planner.js` — можно тонко настроить стиль тренера.
